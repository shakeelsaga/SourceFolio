...
def main():
    ...
    while True:
        ...
        if not list_of_keys:
            console.print("[warn]⚠️ No keywords entered. Please try again.[/warn]")
            sys.exit(1)
        ...
        for key, value in data.items():
            ...
            console.print(
                f"[secondary]Keyword:[/secondary] {key} → [primary]{status}[/primary]"
            )
        console.rule()
        ...
        for keyword in list_of_keys:
            ...
            try:
                books = utils.fetch_with_progress(
                    f"Gathering book data for '{keyword}'", olib.get_books, keyword
                )
                ...
                utils.status_update(
                    f"Error fetching books for '{keyword}'", success=False
                )

            # News API
            try:
                news_data = utils.fetch_with_progress(
                    f"Gathering news data for '{keyword}'", news.get_news, keyword
                )
                ...
                utils.status_update(
                    f"Error fetching news for '{keyword}'", success=False
                )
        ...
        restart = inquirer.confirm(
            message="Do you want to perform another research?", default=False
        ).execute()
        if not restart:
            console.print(
                "[secondary]Exiting. Thank you for using SourceFolio![/secondary]"
            )
