...
class ReportDocTemplate(BaseDocTemplate):
    def __init__(self, filename, **kw):
        ...
        frame = Frame(cm, cm, self.pagesize[0] - 2*cm, self.pagesize[1] - 2*cm, id='normal')
...
def export_to_pdf(data, filename="research_output.pdf"):
    ...
    toc = TableOfContents()
    toc.levelStyles = [
        ParagraphStyle(name='TOCHeading1', fontName='Times-Bold', fontSize=14,
            leftIndent=20, firstLineIndent=-20, spaceBefore=5),
        ParagraphStyle(name='TOCHeading2', fontSize=12,
            leftIndent=40, firstLineIndent=-20, spaceBefore=0),
    ]
    toc_heading_style = ParagraphStyle(name='TOCMainHeading', fontSize=16, leading=20, alignment=0, fontName='Helvetica-Bold', spaceAfter=6)
    ...
    story.append(HRFlowable(width="99%", thickness=1, color="black", hAlign='LEFT', spaceBefore=2, spaceAfter=10))
    story.append(toc)
    ...
    for i, (key, sections) in enumerate(data.items(), start=1):
        ...
        # Summary / Details
        story.append(Paragraph(f"Full Details" if sections['wiki']['is_detailed'] else "Summary", h2))
